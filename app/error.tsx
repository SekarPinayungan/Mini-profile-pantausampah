'use client';

import { useEffect } from 'react';
import Link from 'next/link';

export default function Error({
  error,
  reset,
}: {
  error: Error & { digest?: string };
  reset: () => void;
}) {
  useEffect(() => {
    console.error('[Error Boundary]', error);
  }, [error]);

  return (
    <section className="py-5 d-flex align-items-center" style={{ minHeight: '60vh' }}>
      <div className="container text-center">
        <div className="mb-4">
          <svg xmlns="http://www.w3.org/2000/svg" width="56" height="56" fill="currentColor"
            className="bi bi-exclamation-triangle text-warning" viewBox="0 0 16 16">
            <path d="M7.938 2.016A.13.13 0 0 1 8.002 2a.13.13 0 0 1 .063.016.146.146 0 0 1 .054.057l6.857 11.667c.036.06.035.124.002.183a.163.163 0 0 1-.054.06.116.116 0 0 1-.066.017H1.146a.115.115 0 0 1-.066-.017.163.163 0 0 1-.054-.06.176.176 0 0 1 .002-.183L7.884 2.073a.147.147 0 0 1 .054-.057z"/>
            <path d="M7.002 12a1 1 0 1 1 2 0 1 1 0 0 1-2 0zM7.1 5.995a.905.905 0 1 1 1.8 0l-.35 3.507a.552.552 0 0 1-1.1 0L7.1 5.995z"/>
          </svg>
        </div>
        <h2 className="fw-bold mb-2">Terjadi Kesalahan</h2>
        <p className="text-muted mb-4">
          {error?.message ?? 'Kami mengalami kendala teknis. Coba lagi beberapa saat.'}
        </p>
        <div className="d-flex gap-3 justify-content-center">
          <button onClick={reset} className="btn btn-success rounded-pill px-4">
            Coba Lagi
          </button>
          <Link href="/" className="btn btn-outline-secondary rounded-pill px-4">
            Kembali ke Beranda
          </Link>
        </div>
      </div>
    </section>
  );
}
